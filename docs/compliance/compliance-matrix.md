...
| C4 | Delivery within **5 days same city, 10 days different city**, from advance payment | `app_order.is_same_city`, `advance_paid_at`, `delivery_deadline_at`; stamped by [`payment.service.ts`](../../api/src/payment/payment.service.ts) when an advance settles | same | **Enforced** |
| C5 | Delivery timeline must be **buyer-visible** | `delivery_deadline_at` persisted and returned; tracking screen surfaces it as a countdown, in Bangla | [accessibility baseline](../ux/accessibility-baseline.md) | Backend done, UI pending |
